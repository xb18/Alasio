 ...
 * index.html so they cover every inline script (including the sveltekit
 * bootstrap script, whose content changes on every build).
 *
 ...
 * middleware cannot help here. Run after `vite build` (see
 * package.json build script).
 */
const root = fileURLToPath(new URL("../..", import.meta.url));
const indexHtml = `${root}build/index.html`;
