...
//
// Run with: npm run fix:lucide (scans renderer/)
// or: tsx scripts/lucide-per-icon.ts [dir...] [--check]
//   dir     Directory to scan, relative to the webapp root or absolute.
//           Defaults to "renderer". May be given multiple times.
...
const LUCIDE_INDEX = join(webappRoot, "node_modules", "@lucide", "svelte", "dist", "icons", "index.js");
const LUCIDE_ALIASES = join(webappRoot, "node_modules", "@lucide", "svelte", "dist", "aliases", "aliases.js");
const LUCIDE_ICONS_DIR = join(webappRoot, "node_modules", "@lucide", "svelte", "dist", "icons");
const LUCIDE_MODULE = "@lucide/svelte";
const DEFAULT_DIRS = ["renderer"];
...
function assertInsideProject(root) {
  const rel = relative(webappRoot, root);
  if (rel === ".." || rel.startsWith(`..${sep}`)) {
    console.error(
      `[error] ${root} is outside the project root ${webappRoot}. This script only fixes files inside its own project.`,
    );
    process.exit(2);
  }
}
...
function printHelp() {
  console.log(
    [
      ...
      "",
      "Directories are resolved relative to the webapp root; absolute",
      "paths are accepted too, but must stay inside the webapp project.",
...
function main() {
  ...
  for (const dir of scanDirs) {
    const root = resolve(webappRoot, dir);
